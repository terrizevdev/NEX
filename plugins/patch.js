  ...
  async (message, args) => {
    try {
      ...
      if (url.toLowerCase().includes("https")) {
        ...
        return await message.send("`ANONYMOUS-MD- UPDATE WAS SUCCESSFULL ✅`");
      } else {
...
astro_patch.smd(
  ...
  async (context) => {
    ...
    await context.send(
      `*ANONYMOUS-MD ping is: ${pingTime} ᴍs*`,
      { edit: messageKey },
      "",
      context
    );
      ...
      const designs = [
        async () => {
          const imageBuffer = await axios.get(
            "https://files.catbox.moe/aid1i4.jpg",
            {
          ...
          const captionText = `ANONYMOUS-MD 1.𝟶.𝟶 ᴘᴀᴛᴄʜ\n\n*Ping:* ${pingSeconds} seconds${quoteText}\n\nANONYMOUS-MD`;
...
smd(
  ...
  async (m) => {
    try {
      ...
      const message = `*ANONYMOUS-MD ʀᴜɴɴɪɴɢ sɪɴᴄᴇ:* ${uptimeHours}h ${uptimeMinutes}ᴍ ${uptimeSeconds}s\n \t_ʜᴇʀᴇ's ᴍᴏʀᴇ ɪɴғᴏ_\n*ʀᴀᴍ ᴜsᴀɢᴇ:* ${ramUsage.toFixed(2)} MB\n*ᴄᴘᴜ ɴᴀᴍᴇ:* ${cpuModel}`;
      const contextInfo = {
        isForwarded: true,
        forwardingScore: 999,
        title: "ANONYMOUS-MD Running Since",
        body: message,
        footerText: "ANONYMOUS-MD 2025",
    ...
    try {
      ...
      if (!isNaN(_0x56b5d3)) {
        if (
          ...
          _0x1eb88a[1][_0x210c92(362)]("ANONYMOUS-MD_FANCY_TEXT")
        ) {
