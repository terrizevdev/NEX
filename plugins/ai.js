const {
  ...
  smd(
    ...
    async (message, input) => {
      try {
        ...
        const apiUrl = `https://apis.davidcyriltech.my.id/removebg?url=${encodeURIComponent(url)}`;
  ...
  smd(
    ...
    async (message, input) => {
      try {
        ...
        return await message.send(`*ANONYMOUS-MD ʙᴀʀᴅ ᴀɪ:*\n ${result}`, { quoted: message });
      } catch (error) {
  ...
  smd({
    ...
    try {
      ...
      const { result } = res;
      const astro = "ANONYMOUS-MD ɢᴇᴍɪɴɪ ᴀɪ"
      const tbl = "```";
  ...
  smd(
    ...
    async (_0x1c0160, _0x482db1) => {
        let astro = "ANONYMOUS-MDᴄʜᴀᴛ ʙᴏᴛ"
      try {
  ...
  smd(
    ...
    async (m, q) => {
      try {
        ...
        const apiUrl = `https://apis.davidcyriltech.my.id/ai/dalle?text=${encodeURIComponent(q)}`;
  ...
  smd(
    ...
    async (_0x39716c, _0xe79cfd) => {
      try {
        ...
        const _0x14515f =
          "https://apis.davidcyriltech.my.id/ai/dalle?text=" +
          encodeURIComponent(
    ...
    async (_0x2acf30) => {
      try {
        if (
          ...
          if (
            _0x2acf30.reply_text.includes("ANONYMOUS-MD• ᴀɴɴᴏɴʏᴍᴏᴜs ᴍsɢ") &&
            _0x2dfb59[0].includes("ANONYMOUS-MD• ᴀɴɴᴏɴʏᴍᴏᴜs ᴍsɢ") &&
            ...
            try {
              if (_0x2ecd2a) {
                ...
                if (_0x13a11c.toLowerCase().startsWith("reply")) {
                  ...
                  let _0x3f6b59 =
                    "*ANONYMOUS-MD• ʏᴏᴜʀ ᴀɴᴏɴʏ-ᴍsɢ ʀᴇᴘʟʏ*\n\n*_From @" +
