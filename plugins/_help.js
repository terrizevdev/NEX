...
➮Mᴇᴍᴏ - ${formatp(os.totalmem() - os.freemem())} 
      \n *ANONYMOUS-MD SIMPLE WHATSAPP BOT*\n\n ${readmore} 
╭──❰ *ALL MENU* ❱ 
│🎗 Lɪꜱᴛ 
│🎗 Cᴀᴛᴇɢᴏʀʏ 
│🎗 Hᴇʟᴘ 
...
astro_patch.smd(
  ...
  async (message) => {
    ...
    const { key } = await message.reply("*ANONYMOUS-MD Ping!!!*");
...
astro_patch.cmd(
  {
    ...
    filename: __filename,
    use: "<text>",
    desc: "Translates text to the desired language",
  },
  async (message, query) => {
    try {
      const defaultLang = "en";
      let [targetLanguage, ...textParts] = (query || "").split(" ");
      
      targetLanguage = (targetLanguage || defaultLang).toLowerCase();
      const textToTranslate = message.reply_text 
        || textParts.join(" ").trim();

      if (!textToTranslate) {
        return await message.reply(
          `*Provide text to translate\nEx: ${prefix}trt es Hello World*\nReply to a message with *${prefix}trt fr*`
        );
      }
  ...
  async (message, query) => {
    try {
      ...
      if (commandInfo.alias && commandInfo.alias[0]) {
        output.push("*⚙️Alias:* " + commandInfo.alias.join(", "));
      }
      if (commandInfo.desc) {
