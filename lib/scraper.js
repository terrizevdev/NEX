    ...
    axios
      ...
      .then(({ data: html }) => {
        ...
        srcs.forEach((src) => {
          ...
          results.push(src.replace(/236/g, "736"));
        });
        results.shift();
        resolve(results);
      })
...
async function createUrl(filePath, provider = "1") {
  try {
    if (!filePath) {
      return;
    }
    if (!provider || provider === "1" || provider.toLowerCase() === "telegraph") {
      return await TelegraPh(filePath);
    }
    if (provider === "2" || provider.toLowerCase().includes("ugu")) {
  ...
  return new Promise((resolve, reject) => {
    axios
      ...
      .then((res) => {
        ...
        let results = [];
        $(".sdms-search-results__list-wrapper > div > a").each(function (i, el) {
          results.push({
...
function ffmpeg(inputBuffer, ffArgs = [], tmpName = "", ext = "") {
  return new Promise(async (resolve, reject) => {
    try {
      let tmpFile = path.join(__dirname, "./", +new Date() + "." + tmpName);
      let outFile = tmpFile + "." + ext;
      await fs.promises.writeFile(tmpFile, inputBuffer);
      spawn("ffmpeg", ["-y", "-i", tmpFile, ...ffArgs, outFile])
        .on("error", reject)
        .on("close", async (code) => {
          try {
...
function toAudio(buffer, name) {
  return ffmpeg(buffer, ["-vn", "-ac", "2", "-b:a", "128k", "-ar", "44100", "-f", "mp3"], name, "mp3");
}
function toPTT(buffer, name) {
  return ffmpeg(buffer, ["-vn", "-c:a", "libopus", "-b:a", "128k", "-vbr", "on", "-compression_level", "10"], name, "opus");
}
function toVideo(buffer, name) {
  return ffmpeg(buffer, ["-c:v", "libx264", "-c:a", "aac", "-ab", "128k", "-ar", "44100", "-crf", "32", "-preset", "slow"], name, "mp4");
}

const Config = require("../config");
let json;
if (fs.existsSync("./source/" + Config.LANG + ".json")) {
  json = JSON.parse(fs.readFileSync("./source/" + Config.LANG + ".json"));
} else {
  json = JSON.parse(fs.readFileSync("./source/asta.json"));
}
  ...
  return new Promise((resolve, reject) => {
    axios.get("http://qaz.wtf/u/convert.cgi?text=" + text).then(({ data }) => {
      ...
      $("table > tbody > tr").each(function (i, row) {
        list.push({
          name: $(row).find("td:nth-child(1) > span").text(),
          result: $(row).find("td:nth-child(2)").text().trim()
        });
      });
      resolve(list);
    }).catch(reject);
...
function botpic() {
  return new Promise((resolve) => {
    let g = getString("global");
    let pics = ["" + g.pic1, "" + g.pic2, "" + g.pic3, "" + g.pic4, "" + g.pic5, "" + g.pic6];
    const pick = pics[Math.floor(Math.random() * pics.length)];
    resolve(pick);
  });
}

async function isAdmin(client, chatId, userId) {
  let metadata = await client.groupMetadata(chatId);
  let admins = metadata.participants.reduce((acc, p) => {
    if (p.admin) {
      acc.push({ id: p.id, admin: p.admin });
    }
    return acc;
  }, []);
  var found = admins.find((a) => a.id === userId);
  return found;
}
...
async function syncgit() {
  ...
  const git = simpleGit();
  await git.fetch();
  var log = await git.log(["main..origin/main"]);
  return log;
}
...
async function sync() {
  ...
  var out = "";
  log.all.map((c) => {
    out += "● [" + c.date.substring(0, 10) + "]: " + c.message + "\n- By:" + c.author_name + "\n";
  });
  return out;
}
...
function ringtone(query) {
  return new Promise((resolve, reject) => {
    axios.get("https://meloboom.com/en/search/" + query).then((res) => {
      let $ = cheerio.load(res.data);
      let arr = [];
      $("#__next > main > section > div.jsx-2244708474.container > div > div > div > div:nth-child(4) > div > div > div > ul > li").each(function (i, el) {
        arr.push({
...
function adultvid() {
  return new Promise((resolve, reject) => {
    axios.get("https://tikporntok.com/?random=1").then((res) => {
      ...
      const info = {};
      info.title = $("article > h1").text();
      info.source = $("article > div.video-wrapper.vxplayer").attr("data-post") || "Web Not Response";
      info.thumb =
        $("article > div.video-wrapper.vxplayer > div.vx_el").attr("data-poster") ||
      ...
      info.desc = $("article > div.intro").text();
      info.upload = $("article > div.single-pre-meta.ws.clearfix > time").text();
      info.like = $("article > div.single-pre-meta.ws.clearfix > div > span:nth-child(1) > span").text();
      info.dislike = $("article > div.single-pre-meta.ws.clearfix > div > span:nth-child(2) > span").text();
      info.favorite = $("article > div.single-pre-meta.ws.clearfix > div > span:nth-child(3) > span").text();
      info.views = $("article > div.single-pre-meta.ws.clearfix > div > span:nth-child(4) > span").text();
      info.tags = $("article > div.post-tags").text();
      info.video =
...
function hentai() {
  return new Promise((resolve, reject) => {
    ...
    axios.get("https://sfmcompile.club/page/" + page).then((res) => {
      const $ = cheerio.load(res.data);
      const arr = [];
...
async function Insta(url) {
  const out = [];
  try {
    const payload = { url: url, submit: "" };
    const { data } = await axios("https://downloadgram.org/", {
      method: "POST",
      data: payload
    });
    const $ = cheerio.load(data);
    $("#downloadhere > a").each((i, el) => {
...
const mediafireDl = async (url) => {
  ...
  const sizeText = $("a#downloadButton").text().replace("Download", "").replace("(", "").replace(")", "").replace("\n", "").replace("\n", "").replace("                         ", "");
...
function dare() {
  const pools = [dare_normal_questions, more_normal_dare_questions, dirty_dare_questions_part_1, dirty_dare_questions_part_2];
...
function truth() {
  const pools = [embarrassing_questions_truth, truth_girl_questions, tord_friend_questions, truth_guy_questions, flirty_questions, dirty_questions];
  const pickPool = pools[Math.floor(Math.random() * pools.length)];
  const pick = pickPool[Math.floor(Math.random() * pickPool.length)];
  return pick;
}
...
function random_question() {
  const pools = [embarrassing_questions_truth, truth_girl_questions, tord_friend_questions, truth_guy_questions, flirty_questions, dirty_questions, dare_normal_questions, more_normal_dare_questions, dirty_dare_questions_part_1, dirty_dare_questions_part_2];
  const pickPool = pools[Math.floor(Math.random() * pools.length)];
  const pick = pickPool[Math.floor(Math.random() * pickPool.length)];
  return pick;
}
